...
{
    short *lPtr = &array[m2a(left)];
    if (size > max_size || *lPtr > 0x4000)
        return NULL;
    if (size == max_size && *lPtr == 0x4000)
        return (void *)heap_top;
    if (size < min_size)
        size = min_size;
        int mid = (right == mcb_bot) ? ((right + 2) - left) / 2 : (right - left) / 2;
    short *mPtr = &array[m2a(left + mid)];
    if (*lPtr >= size && *lPtr%2 ==0)
    {

        if (*lPtr == size || *lPtr / 2 < size)
        {
            *lPtr += 1;
            int add = (left - mcb_top)*0x10 + heap_top;
            return (void *)add;
        }
        ...
        *lPtr /= 0x2;
        int temp = *lPtr;
        *mPtr = temp;
        return _ralloc(size, left, left + mid);
    }
    if(*lPtr != max_size && left < mcb_bot)
    {
        int buddy = left+(*lPtr/0x10);
        short *bptr = &array[m2a(buddy)];
        return _ralloc(size, buddy, (*bptr/0x10)+buddy);
    }
    return NULL;
}
...
int _rfree(int mcb_addr)
{
    ...
    short *lPtr = &array[m2a(mcb_addr)];
    printf( "_rfree: mcb[%x] = %x\n", mcb_addr, *(short *)&array[ m2a( mcb_addr ) ] );
    *lPtr -= *lPtr%2;

    int mem = *lPtr / 0x10;
    printf( "_rfree: mem= %x, *ptr = %x\n", mem, *lPtr);
    int loc = (mcb_addr - mcb_top) / mem;
    short *ret = (loc % 2) ?&array[m2a(mcb_addr + mem)] : &array[m2a(mcb_addr - mem)];
    if (mcb_addr - mem >= mcb_top && mcb_addr + mem <= mcb_bot)
    {
       
        if (*ret%2 ==0 && *ret != 0)
        {
             //printf( "_rfree: mem= %x, ptr= %x, *ptr = %x, ret= %x, *ret = %x\n", mem, lPtr, *lPtr, ret, *ret);

            *ret = (*ret*2);
            *lPtr = 0;
            return (loc % 2)? _rfree(mcb_addr+mem):_rfree(mcb_addr-mem);
        }
    }
    return mcb_addr;
}
 ...
 int buddy = ( mcb_addr - mcb_top ) / ( mcb_contents / 0x10 );
