...
void *_ralloc( int size, int left, int right ) {
  ...
  short *lPtr = (short *)left;
  printf("_ralloc: left=%x, mem=%x\n", lPtr, &lPtr);
  if(size > max_size || *lPtr > 0x4000) return NULL;
  if(size == max_size && *lPtr == 0x4000) return (void *)heap_top;
  if(size < min_size) size = min_size;

  int mid = (right == mcb_bot)?((right+2)-left)/2:(right-left)/2;
  if(*lPtr%0x2 && (mid-left)*0x10 > size){
    return ((mid * 0x20)-(*lPtr-0x1) >= size)?_ralloc(size, left, (left+mid)):_ralloc(size, (left+mid), right);
  }else if(*(lPtr) >= size)
  {
      printf("_ralloc: mid=%d, leftptr=%x, lmem=%x\n", mid, lPtr, *lPtr );
    if(*lPtr == size || *lPtr/2 < size)
    {
      ++ *lPtr;
     return (void *)((left-heap_top) * 0x10);
    }

    short *mPtr = (short *)(left+mid);
    *lPtr /=0x2;
    int temp = *lPtr;
    *mPtr = temp;
    return _ralloc(size, (int)lPtr, (int)mPtr);
  }
...
int _rfree( int mcb_addr ) {
  ...
  if(mcb_addr < mcb_top || mcb_addr > mcb_bot) return 0;
  short *lPtr = (short*)mcb_addr;
  --*lPtr;
  if(mcb_addr == mcb_top && *lPtr > 0x4000)
  {
    return mcb_addr;
  }
  int mem = *lPtr/0x10;
  int loc = (mcb_addr-mcb_top)/2;
  short * ret = (loc%2)?(short *)(mcb_addr + mem): (short *)(mcb_addr - mem);
